...
import json, select, socket, threading, traceback, logging, time 
...
class Blue():
    def __init__(self):
        ...
        self.t_update_T = threading.Thread(target=self.update_T, args=[])
        self.t_update_T.start()  
        
        self.update()
        
    def info(self, lvl, content):
        if lvl == "N": logging.info("[BLUE/INFO] ({})".format(content))
        
        if lvl == "E": logging.info("[BLUE/ERROR] ({})".format(content))

        if lvl == "W": logging.info("[BLUE/WARNING] ({})".format(content))

    def enter(self, client, username):
        self.info("N", "player {} logged!".format(username))
    ...
    def update(self):
        logging.info("[BLUE/START] listening on: {}:{}\n".format(self.HOST[0], self.HOST[1]))
    ...
    def update_T(self):
        while True:
            info["players-online"] = int(len(self.players))
